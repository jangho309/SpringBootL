...
public class ApiController {
    ...
    public ResponseEntity<?> findById(@Parameter(description = "조회할 Memeber의 id") @PathVariable("id") int id){
        ResponseDto<MemberDto> responseDto = new ResponseDto<>();
    ...
    public ResponseEntity<?> modify(
            ...
            // x-www-form-urlencoded 형태는
            // @ModelAttribute나 @RequestParam으로
            // 데이터를 전송받아 사용했다.
            // 전송되는 데이터의 형태가
            // application/json 형태면
            // @RequestBody 어노테이션을 사용한다.
            @Parameter(description = "Member의 수정될 password, nickname, email, tel") MemberDto memberDto){
        ResponseDto<MemberDto> responseDto = new ResponseDto<>();
